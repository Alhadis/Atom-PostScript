...
const {CompositeDisposable, Emitter, File} = require("atom");
...
class PortalView{
	...
	constructor(state = {}){
		...
		this.editorId
			...
			: this.waitToLoad().then(() => this.watchFile(this.filePath));
	}
	...
	getProtocolName(){
		return this.constructor.protocolName; // HACK
	}
	...
	getTitle(){
		return this.file && this.getPath()
			...
			: this.editor ? this.editor.getTitle() : "Preview";
	}
	...
	getURI(){
		return this.file
			? this.getProtocolName() + "://source:file@"   + this.getPath()
			: this.getProtocolName() + "://source:editor@" + this.editorId;
	}
	...
	static get createView(){
		return state => {
			if(state.editorId || this.isFile(state.filePath))
	...
	static get add(){
		return editor => {
			...
			atom.workspace.open(uri, {searchAllPanes: true}).then(view => {
				if(view instanceof this)
					pane.activate();
			});
		};
	}
